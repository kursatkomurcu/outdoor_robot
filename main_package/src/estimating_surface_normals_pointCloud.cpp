...
void publishOccupancyGrid(const pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr cloud_with_normals, float thresholdDegree)
{
    ...
    for (const auto& point : *cloud_with_normals) {
        ...
        float slope = atan2(sqrt(point.normal_x * point.normal_x + point.normal_y * point.normal_y), point.normal_z) * 180 / M_PI;
        ...
        //std::cout << "Slope Degree: " << slope << std::endl;
        if (slope >= thresholdDegree) {
...
int main(int argc, char** argv)
{
    ...
    ros::Rate rate(5);
    nh.param<float>("thresholdDegree", thresholdDegree, 10.0);
    nh.param<float>("distance", distance, 10.0);
    ...
    sub = nh.subscribe<sensor_msgs::PointCloud2>(IMAGE_TOPIC, 1, cloud_cb);

    pub = nh.advertise<sensor_msgs::PointCloud2>(PUBLISH_TOPIC, 1);
    occupancyGridPub = nh.advertise<nav_msgs::OccupancyGrid>(OCCUPANCY_GRID_TOPIC, 1);
