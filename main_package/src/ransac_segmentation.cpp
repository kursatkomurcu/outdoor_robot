...
static const std::string LASER_MOVE_BASE_TOPIC = "/ransac_cloud";
...
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
    ...
    seg.setDistanceThreshold(0.55);
    seg.setInputCloud(cloud_pt);
    seg.segment(*inliers, *coefficents);

    pcl::ExtractIndices<pcl::PointXYZRGB> extract;
    extract.setInputCloud(cloud_pt);
    ...
    publishOccupancyGrid(roadCloud, thresholdDegree);

    // Convert the pcl::PointCloud<pcl::PointXYZRGB> to sensor_msgs::PointCloud2
    sensor_msgs::PointCloud2 output;
    pcl::toROSMsg(*roadCloud, output);
